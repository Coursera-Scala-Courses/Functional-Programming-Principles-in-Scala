...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    if (that.equals(new Empty)) this
    else right.union(left.union(that.incl(this.elem)))
  }

  def mostRetweeted: Tweet = {

    var mostRetweetedTweet :Tweet = this.elem

    def setMostRetweetedTweet(tweet: Tweet): Unit ={
      if(tweet.retweets>mostRetweetedTweet.retweets) mostRetweetedTweet = tweet
    }

    foreach(setMostRetweetedTweet)
    mostRetweetedTweet
  }
